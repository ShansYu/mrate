...
model = JODIE(args, num_features, num_users, num_items).cpu()
weight = torch.Tensor([1,true_labels_ratio]).cpu()
crossEntropyLoss = nn.CrossEntropyLoss(weight=weight)
MSELoss = nn.MSELoss()

# INITIALIZE EMBEDDING
initial_user_embedding = nn.Parameter(F.normalize(torch.rand(args.embedding_dim).cpu(), dim=0)) # the initial user and item embeddings are learned during training as well
initial_item_embedding = nn.Parameter(F.normalize(torch.rand(args.embedding_dim).cpu(), dim=0))
...
item_embedding_static = Variable(torch.eye(num_items).cpu()) # one-hot vectors for static embeddings
user_embedding_static = Variable(torch.eye(num_users).cpu()) # one-hot vectors for static embeddings
# user_neighbor_embeddings = initial_user_neighbor_embedding.repeat(1, num_neighbor, 1)
# item_neighbor_embeddings = initial_item_neighbor_embedding.repeat(1, num_neighbor, 1)
...
with trange(args.epochs) as progress_bar1:
    for ep in progress_bar1:
        ...
        user_embeddings_timeseries = Variable(torch.Tensor(num_interactions, args.embedding_dim).cpu())
        item_embeddings_timeseries = Variable(torch.Tensor(num_interactions, args.embedding_dim).cpu())

        optimizer.zero_grad()
        reinitialize_tbatches()
        initialize_neighbors_dicts()
                ...
                if timestamp - tbatch_start_time > tbatch_timespan:
                    ...
                    with trange(len(lib.current_tbatches_user)) as progress_bar3:
                        for i in progress_bar3:
                            ...
                            tbatch_userids = torch.LongTensor(lib.current_tbatches_user[i]).cpu() # Recall "lib.current_tbatches_user[i]" has unique elements
                            tbatch_itemids = torch.LongTensor(lib.current_tbatches_item[i]).cpu() # Recall "lib.current_tbatches_item[i]" has unique elements
                            tbatch_interactionids = torch.LongTensor(lib.current_tbatches_interactionids[i]).cpu() 
                            feature_tensor = Variable(torch.Tensor(lib.current_tbatches_feature[i]).cpu()) # Recall "lib.current_tbatches_feature[i]" is list of list, so "feature_tensor" is a 2-d tensor
                            user_timediffs_tensor = Variable(torch.Tensor(lib.current_tbatches_user_timediffs[i]).cpu()).unsqueeze(1)
                            item_timediffs_tensor = Variable(torch.Tensor(lib.current_tbatches_item_timediffs[i]).cpu()).unsqueeze(1)
                            tbatch_itemids_previous = torch.LongTensor(lib.current_tbatches_previous_item[i]).cpu()
